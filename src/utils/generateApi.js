...
router.route("/")
    .post(requireAuth, add${modelName})
    .get(requireAuth,getAll${modelName}s);

router.route("/:id")
    .get(requireAuth,get${modelName})
    .put(requireAuth, put${modelName})
    .delete(requireAuth, delete${modelName});
...
fs.readFile(indexFilePath, "utf8", (err, data) => {
  ...
  const newRouteExport = `\nexports.${modelName.toLowerCase()}Routes=require("./${routeFileName}")`

  // Remove the last "}" from the module.exports object
  const updatedData = data + newRouteExport
